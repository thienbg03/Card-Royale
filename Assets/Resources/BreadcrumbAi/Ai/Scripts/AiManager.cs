...
namespace BreadcrumbAi{
	public static class AiManager {
		...
		static private GameObject[] players, enemies, breadcrumbs, waypoints;
        ...
        */
        ...
        public static IEnumerator Ai_Lists(this Ai ai){
			while(ai.enabled){
				...
				enemies = GameObject.FindGameObjectsWithTag(enemyString);
		...
		*/
		...
		/*
			Custom modification
			If ai is an ally, switch player target to enemy
			Ai will focus the first target till death
		 */
		public static Transform Ai_FindPlayer(this Ai ai) {
			if (ai._IsAlly)
			{
                if (ai.Player)
                {
					if(ai.Player.gameObject != null || ai.Player.gameObject.activeSelf != false)
					{
                        return ai.Player;
                    }
					return null;
                    ...
                    //    {
                    //        if (hit.collider.tag == enemyString)
                    //        {
